...
class GaussianTrainer:
    ...
    def visualize_rendering(self, dataset, save_vid_path: str, num_frames: int = 300):
        ...
        with torch.no_grad():
            ...
            gaussian_params = {
                k: torch.nan_to_num(v, nan=0.0, posinf=1e6, neginf=-1e6)
    ...
    def _sanitize_gradients(self):
        for p in self.model.parameters():
            if p.grad is not None:
                p.grad = torch.nan_to_num(p.grad, nan=0.0, posinf=1e3, neginf=-1e3)
    ...
    def train_step(self, batch: dict, in_train = True) -> float:
        ...
        if not torch.isfinite(loss):
            print("WARNING: non-finite loss detected, skipping optimizer step")
...
def parse_args():
    ...
    # Data reduction for CPU training
    parser.add_argument('--downsample_factor', type=int, default=8,
                      help='Image downsample factor for faster CPU training')
    parser.add_argument('--maximum_pts_num', type=int, default=3000,
                      help='Maximum number of 3D points to use')
