...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num = 3000):
        ...
        # Sample 3D points to a specific number for faster CPU training
        if maximum_pts_num is not None and maximum_pts_num > 0 and self.points3D_xyz.shape[0] > maximum_pts_num:
            indices = self._furthest_point_sampling(self.points3D_xyz, maximum_pts_num)
    ...
    @staticmethod
    def _furthest_point_sampling(points: torch.Tensor, k: int) -> torch.Tensor:
        """Simple CPU-friendly furthest point sampling."""
        n = points.shape[0]
        indices = [0]
        selected = points[0:1]
        dist = torch.cdist(points, selected, p=2).squeeze(1)
        for _ in range(1, k):
            next_idx = torch.argmax(dist).item()
            indices.append(next_idx)
            selected = torch.cat([selected, points[next_idx:next_idx+1]], dim=0)
            new_dist = torch.cdist(points, selected[-1:], p=2).squeeze(1)
