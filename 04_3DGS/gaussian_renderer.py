...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        depths = torch.nan_to_num(depths, nan=1e-4, posinf=1e6, neginf=-1e6)
        
        # 3. Project to screen space using camera intrinsics
        screen_points = cam_points @ K.T  # (N, 3)
        z = screen_points[..., 2:3]
        z_safe = z.clamp(min=1e-4)
        means2D = screen_points[..., :2] / z_safe # (N, 2)
        means2D = torch.nan_to_num(means2D, nan=0.0, posinf=1e6, neginf=-1e6)
        ...
        covs2D = torch.nan_to_num(covs2D, nan=1e-6, posinf=1e6, neginf=-1e6)
    ...
    def compute_gaussian_values(
        ...
        power = torch.clamp(power, max=0.0, min=-100.0)
    ...
    def forward(
        ...
        gaussian_values = torch.nan_to_num(gaussian_values, nan=0.0, posinf=0.0, neginf=0.0)
