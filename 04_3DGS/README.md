...
python render_3dgs_mv.py \
    ...
    --output data/chair/checkpoints/render_mv_high_quality.mp4 \
    --num_frames 300 --fps 30 --device cpu \
    --downsample_factor 8 --maximum_pts_num 2000
```

默认输出路径为 `data/chair/checkpoints/render_mv_high_quality.mp4`，请保证渲染参数与训练时的数据下采样和点数一致。
